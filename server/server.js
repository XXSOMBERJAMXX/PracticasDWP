...
app.post('/register', async (req, res) => {
  ...
  if (!username || !email || !password || username.includes(' ') || email.includes(' ') || password.includes(' ')) {
    return res.status(400).json({ success: false, message: "Todos los campos son obligatorios y no deben contener espacios." });
  }
...
app.get('/tasks', verifyToken, async (req, res) => {
  try {
    ...
    let query = {};
    if (groupId) {
      query.group = groupId; // Filtrar tareas por grupo
    }else{
  ...
  try {
    ...
    if (groupId) {
      ...
      if (!role.permissions.includes("create_tasks")) {
        return res.status(403).json({ success: false, message: "No tienes permisos para crear tareas en este grupo" });
      }
    }
...
// Actualizar tarea verificando pertenencia
...
app.post('/groups/:groupId/add-member', verifyToken, async (req, res) => {
  try {
    ...
    if (isMember) {
      return res.status(400).json({ success: false, message: "El usuario ya es miembro del grupo" });
    }
...
app.delete('/groups/:groupId', verifyToken, async (req, res) => {
  try {
    ...
    const tasks = await Task.find({ group: req.params.groupId });
    if (tasks.length > 0) {
      return res.status(400).json({ success: false, message: "No se puede eliminar el grupo porque tiene tareas asociadas" });
    }
...
app.get('/users', verifyToken, async (req, res) => {
  try {
    ...
    if (!username) {
      return res.status(400).json({ success: false, message: "Se requiere un username" });
    }
...
app.delete('/groups/:groupId/remove-member/:userId', verifyToken, async (req, res) => {
  try {
    ...
    if (memberIndex === -1) {
      return res.status(404).json({ success: false, message: "El usuario no es miembro del grupo" });
    }
...
app.put('/groups/:groupId', verifyToken, async (req, res) => {
  try {
    ...
    if (group.createdBy.toString() !== req.userId) {
      return res.status(403).json({ success: false, message: "No tienes permisos para actualizar este grupo" });
    }
